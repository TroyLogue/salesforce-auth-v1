...
module MailtrapHelper
  ...
  def find_reset_link(str)
    ...
    str.match(%r{#{auth_url}/secret/edit\?client_id=\w{10}&amp;reset_password_token=\S{20}})[0]
  end

  def find_share_link(str)
    base_url = 'https://bit.ly'.gsub('/', '\/')
    str.match(%r{#{base_url}/\w{7}})[0]
  end
  ...
  def get_messages(filter: '', mailbox_id: ENV['MAILTRAP_ID'])
    ...
    if response.status.success?
      ...
      raise StandardError, "Call to mailtrap API failed with status #{response.status}"
    end
  end
