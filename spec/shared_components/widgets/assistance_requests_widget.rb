...
class AssistanceRequestWidget < BasePage
  ...
  HEADER = { css: 'h3' }
  LINK = { css: 'a'}
  SUBMITTING_TEXT = { css: '#container > div > div > div:nth-child(7)' }
  ...
  def submit_form_with_all_fields(
    ...
    enter(dob, DOB_INPUT)
    click(PHONE_INPUT)
    enter(VALID_PHONE_NUMBER, PHONE_INPUT)
    ...
    enter(ssn, SSN_INPUT)
    enter( GROSS_INCOME, GROSS_MONTHLY_INCOME_INPUT)
    click(MILITARY_AFFILIATION_DROPDOWN)
    click(MILITARY_AFFILIATION_FIRST_OPTION)
    enter(SCHOOL_NAME, SCHOOL_INPUT)
    enter(SCHOOL_LOCATION, SCHOOL_LOCATION_INPUT)
