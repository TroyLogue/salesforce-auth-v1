...
describe '[Referrals - External]', :app_client, :referrals do
  ...
  context('[as a Referral User and Out of Network Cases User]') do
    ...
    it 'creates OON referral to external and custom providers using primary worker search', :uuqa_1633, :uuqa_1901 do
      ...
      oon_case_selections = add_referral_page.select_first_oon_options(description: Faker::Lorem.sentence(word_count: 5))
      custom_org_name = Faker::Educator.university
      add_referral_page.add_custom_org(name: custom_org_name)
      add_referral_page.click_create_case
      ...
      expect(case_detail_page.referred_to_array).to contain_exactly(oon_case_selections[:org], custom_org_name)
      expect(case_detail_page.primary_worker).to eq oon_case_selections[:primary_worker]
    end
  end
end
