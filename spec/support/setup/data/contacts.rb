...
module Setup
  module Data
    def self.create_harvard_client
      contact = Contact.new(token: Auth.access_token(email_address: Users::CC_USER), group_id: Providers::CC_HARVARD)
      contact.create
      contact
    end
    ...
    def self.create_princeton_client
      contact = Contact.new(token: Auth.access_token(email_address: Users::ORG_PRINCETON), group_id: Providers::ORG_PRINCETON)
      contact.create
      contact
    end
    ...
    def self.select_client_in_princeton(contact:)
      contact.select(token: Auth.access_token(email_address: Users::ORG_PRINCETON), group_id: Providers::ORG_PRINCETON)
      contact
    end

    def self.create_yale_client
      contact = Contact.new(token: Auth.access_token(email_address: Users::ORG_YALE), group_id: Providers::ORG_YALE)
      contact.create
      contact
    end
    ...
    def self.create_columbia_client
      contact = Contact.new(token: Auth.access_token(email_address: Users::ORG_COLUMBIA), group_id: Providers::ORG_COLUMBIA)
      contact.create
      contact
    end
